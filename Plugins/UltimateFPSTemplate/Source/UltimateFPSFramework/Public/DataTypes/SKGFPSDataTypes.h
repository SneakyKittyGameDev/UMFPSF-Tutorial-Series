...
struct FSKGFirearmPoseCurveSettings
{
	...
	FTransform GetTransformMultiplied(const float CurveTime, bool bLocallyControlled, float Multiplier = 1.0f) const
	{
		FTransform Transform;
		if (IsValid())
		{
		...
		Transform.SetLocation(Transform.GetLocation() * Multiplier);
		Transform.SetRotation((Transform.Rotator() * Multiplier).Quaternion());
		return Transform;
	}
